...
if ($item_validasi[$i] == 1) {
    ...
    $sql="update tbl_tracking set posisi = 'st_off' where id_item =".$arr_barcode[1]." and posisi_item = ".$arr_barcode[2];
    insert($sql,"ra");
    
    /* Log process into TKO */
    
    $sql="insert into log_tracking_item (dateinsert, id_item, posisi_item, id_officer, posisi) values(NOW(),'".$arr_barcode[1]."','".$arr_barcode[2]."', '".$_SESSION['id_user']."', 'st_off')";
    ...
    echo "<script>window.location='/?opsi=sto_off_tracking'</script>";
...
foreach ($arr_result as $op) {
    ...
    $pesan = "Kepada $arr_perusahaan[$v]%5Cn%5CnSMU : $smu%5CnFlight : $arr_flight[$v]%5CnTLP : $arr_telp[$v]%5Cn%5CnItem :%5Cn$arr_item[$v]%5Cn%5CnPOS : Arrival";
